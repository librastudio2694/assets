...
function multifungsi(){
  ...
  function kalimatakhir(){
      ...
      afterComplete: function () {
            ...
            setTimeout(() => {
                munculteksnim();
                setTimeout(() => {
                    document.getElementById("Tombol1").classList.add("show");
                }, 300); // Delay tambahan untuk tombol
            }, 300);
        },
    }).go();
}
  ...
  function bqmuncul(){
    if(poinjwb==1){
      ...
      klganti.innerHTML="Udah lah segitu aja 🤣<br><br>";katakata = kalimat.innerHTML;kalimat.innerHTML = "";
    }
  ...
  async function aksibalas(){
    ...
    if(prtanya){
       await swals.fire({
         title: 'Tapi Boong! 🤣', 
         html: 'Gajadi ngasih kado dehh<br>soalnya kamu nya sudah terlalu sempurna 😂', 
